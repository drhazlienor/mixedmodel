...
```{r}
# install.packages(c("readxl", "tidyverse", "lme4", "lmerTest", "gtsummary", "sjPlot", "lattice"), dependencies = TRUE)
```
...
Lets do simple and multiple linear regression to unders
...
Lets calculate the ICC for the **model_ast_ri:**

```{r}
1.429/(1.429+3.079)
```

About **31.7%** of the variability in the **LOS** (day) is attributable to differences between hospitals, while the remaining **68.3%** is due to within-hospital variability not explained by the model (residuals).

#### **Preliminary Final Model**

We select the model with lower AIC/BIC and improved log-likelihood as the preliminary final model: **model_ast_rs.**

Lets check the model fitness:
...
```{r}
tbl_regression(model_ast_rs)
```

#### Inference

or obtain the estimates and confidence interval for each predictors using broom.mixed
...
```{r}
tidy(model_ast_rs, conf.int=TRUE)
```

extract the random effect for each hospital site

```{r}
ra.eff.rs <- ranef(model_ast_rs, condVar = TRUE)
ra.eff.rs
```
...
For each year increase in age, the length of hospital stay increase by 0.009 days (95% CI: -0.01, 0.01, p-value = 0.397), when adjusted for SOFA score, therapy type, random effect of hospital, and random effect of therapy type, although the association is not significant.

**Random effect interpretation:**

The analysis demonstrated variability in the association between therapy type and length of hospital stay on patient with MDR infection receiving carbapenem across different hospitals (random intercept variance = 1.429 days**²,** sd = 1.195 days).

The weak correlation(0.07) between the random intercepts and random slopes suggests that hospitals with a longer baseline length of hospital stay do not necessarily show a stronger or weaker effect of monotherapy.

About 31.7% of the variability in the length of hospital stay is due to differences between hospital. The remaining 68% (residual variance (3.079, sd = 1.755) reflects the unexplained variation in length of hospital stay after accounting for both fixed and random effects. This suggests that other unmeasured factors, such as patient characteristics, treatment protocols, or hospital resources, likely influence length of hospital stay.
...
ran.int <- ranef(model_ast_rs)$hospital_id  
...
Plot the random slope and random intercept
...
```{r}
randoms <- ranef(model_ast_rs, condVar = TRUE)
dotplot(randoms)
```
